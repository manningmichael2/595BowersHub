...
async def handle_list(params: dict) -> dict:
    ...
    if action == "all":
        ...
        return await add_items(list_name, items)
    elif action in ("check", "done", "bought", "got", "purchased"):
        return await check_items(list_name, items)
    elif action in ("remove", "delete"):
        return await remove_items(list_name, items)
    elif action in ("clear", "clean"):
        return await clear_checked(list_name)
    else:
        # Default: view the list
        show_checked = params.get("show_checked", False)
        return await get_list(list_name, show_checked=show_checked)
