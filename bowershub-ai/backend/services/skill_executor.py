...
class SkillExecutor:
    ...
    async def _user_meets_min_role(self, skill: dict, user_id: int) -> bool:
        """Return True if the user's role rank satisfies the skill's `min_role`.

        NULL/empty min_role → no restriction. Unknown roles rank 0 (least
        privileged), so a typo'd min_role fails closed rather than open."""
    ...
    async def execute(
        ...
        # DB-driven per-skill minimum-role gate (bh_skills.min_role).
        if not await self._user_meets_min_role(skill, user_id):
            raise SkillPermissionError(
                f"Skill '{skill_name}' requires the '{skill['min_role']}' role"
            )
