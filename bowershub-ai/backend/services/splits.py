...
async def create_split(conn, txn_id: str, allocations: list[dict]) -> dict:
    ...
    async with conn.transaction():
        ...
        for cat_id, amt in alloc:
            await conn.execute(
                ...
                " user_category_override, source) "
                "VALUES (gen_random_uuid()::text, $1, $2, $3, $4, $5, $6, $7, 'split')",
                parent["account_id"], parent["posted_date"], amt, parent["description"],
                cat_id, txn_id, cat_id is not None)
    return {"transaction_id": txn_id, "children": len(alloc)}


async def unsplit(conn, txn_id: str) -> dict:
    ...
    async with conn.transaction():
        ...
        await conn.execute(
            "UPDATE finance.transactions "
            "SET is_split = false, user_category_override = false, updated_at = now() "
            "WHERE id = $1", txn_id)
