...
## [2026-06-28] Settings sweep, round 2 — merged+deployed; web push explained; notification-architecture gap found — Claude Code

Owner: "merge and redeploy … continue / keep sweeping." Merged the settings full pass (`4270cb1`), redeployed (health ok, un-staled the backend again). Then kept sweeping; three more changes, all merged to `main` + deployed.

- **Web push, clarified for the owner (decision logged):** owner will **stay on Pushover** (already configured). Web push remains wired but needs server VAPID keys + a per-device subscription; Pushover is the simpler working channel.
- **WorkspaceSettingsPanel cleanup (`aebc73f`):** the System-Prompt viewer/editor + PinnedContextManager were hidden behind a `useOptionalChild` + `import(/* @vite-ignore */)` indirection with dead "ships with task 22.2/22.3/23.1" placeholders (those tasks shipped). Replaced with plain static imports; editor `onSaved`→panel cached prompt. Those components pull heavy markdown/editor deps and the dynamic import had also code-split them, so made the **whole modal a `React.lazy` boundary in `Sidebar`** (mounts on open) — main bundle stayed ~875KB/gzip255 (was about to grow to gzip314), panel is a ~60KB-gzip lazy chunk.
- **"Send test notification" button + send() accuracy fix (`dad4298`):** new `POST /api/me/notifications/test` goes through the real `send()` path (honors prefs + quiet hours) so the owner can confirm delivery. While wiring it, **fixed a pre-existing bug**: `send()` set `sent=True` after `_send_web_push` even when web push was unconfigured/no device (helper returned early without raising) → both over-reported success AND suppressed the Pushover fallback. `_send_web_push` now returns whether it actually dispatched; `send()` only counts it then.

- **⚠️ ARCHITECTURE FINDING (owner decision needed) — the Notifications settings govern almost nothing today.** There are **two parallel notification systems**: (1) `services/notifications.py::NotificationService.send()` carries the per-user prefs (web_push/pushover/quiet_hours, default-row fallback) — but is called **only by the new test endpoint**; (2) `services/pushover.py::send_notification()` posts straight to the **single household Pushover account** (env `PUSHOVER_USER_KEY`) with no user_id, no prefs, no quiet hours — and **this is what actually fires** budget/inbox/reminder alerts (`services/alerts.py`, scheduled from `main.py`) + gameday + the hook_engine scheduled-prompt push. So the Notifications panel's channel toggles + quiet hours **do not affect the real alerts** a user receives. Fixing it is a household design call (route alerts through `send()` per-user → but budgets are *shared*, so *who* gets the alert + *whose* quiet hours? — needs an owner decision before building). Not started; flagged.

- **Verified clean (no action):** global toast IS wired (`components/Toaster.tsx` mounted in `main.tsx`) — all the new toasts render. Frontend source has zero TODO/FIXME/mock/hardcoded-data markers; the "outdated settings" smell was the placeholder sections (now fixed) + the workspace-panel cruft (now fixed).

- [Next] **Owner to decide on the notification-architecture gap** (per-user routing of shared alerts vs keep single-household-account + maybe honor a household quiet-hours). Then optionally: per-event notification granularity (table is keyed by `event_type`), DB-driven per-skill `min_role`, C6 `any`-at-API-boundary cleanup. All settings-sweep code is merged + live; no migration this round.

## [2026-06-28] Per-user notification routing — closed the architecture gap — Claude Code

Owner chose **per-user routing** for the gap flagged above. Built + merged (`c63991a`) + deployed (migration 0046 applied; health ok). Backend-only.

- **`NotificationService.notify_users(user_ids, event_type, …)`** — the real alerts now go through this instead of `pushover.send_notification` (single shared account). **Web push is genuinely per-user** (each member's registered devices, honoring *their* quiet hours); **Pushover targets the ONE shared household account** (single `PUSHOVER_USER_KEY`), so it fires **at most once per event** even when several recipients opted in — no duplicate buzzes. It fires only for awake (non-quiet-hours) opted-in users. Returns a delivery summary (`delivered`/`attempted`/`web_push_count`/`pushover_sent`).
- **Recipients (`services/alerts.py` helpers `_all_active_user_ids`/`_finance_user_ids`):** reminders → the reminder's own `user_id` (was mis-routed to the shared account — `bh_reminders.user_id` is NOT NULL); budgets → active users resolving `finance.read` (shared household); inbox + gameday → all active users. Wired budgets/inbox/reminders (`alerts.py`) + gameday (`gameday_alerts.py`).
- **Reminder delivery semantics:** mark `delivered_at` once *attempted for an awake user* (not just on channel success), so a quiet-hours user's reminder waits for the window to end rather than looping forever or dropping. `gameday_alerts` now imports `_all_active_user_ids`/`_get_notifier` from `alerts` (no cycle).
- **Continuity — migration `0046_seed_notification_pushover_defaults.sql`:** the prefs fallback for a user with no row is web_push-on/**pushover-OFF**, so without seeding, existing members would silently stop getting Pushover after the cutover. Seeds each active user a `default` row with pushover=on (matches today). Idempotent (`ON CONFLICT DO NOTHING`) — never clobbers a pref already set in Settings. New users get the opt-in default until they enable Pushover themselves.
- **Note:** `services/pushover.py::send_notification` is now largely orphaned (only healthcheck uses pushover.net directly + hook_engine has its own direct POST at line ~640). Left in place; a later cleanup could route the hook_engine scheduled-prompt push through `notify_users` too (currently still the owner-targeted shared-account path via `send_pushover`).
...
## [2026-06-28] Assistant capabilities pass — slash cleanup + skills (lists, read-email) — Claude Code

Owner asked to "move ahead" on (1) new skills and (2) slash-command cleanup; chose a **native** household list (not Grocy) and **no spec session** (these are thin adds/wiring, not finance-north-star-sized). Branch `feat/assistant-skills-and-cleanup` (5 commits). Throwaway pg on host 55433 for DB tests + a fresh-migrate audit script.

**Key discovery: most of #1 was already built and just dormant/unwired.** A migrated-DB audit of `bh_slash_commands` ⋈ `bh_skills` drove the cleanup and surfaced this.

- **#2 slash dedup (`0047`)** — `/schedule` was seeded **twice** (ids 24 & 26, identical); no uniqueness guard existed. De-dupes global (`workspace_id IS NULL`) commands keeping the lowest id + adds a partial unique index so it can't recur. Workspace-scoped names untouched.
- **Grocery/shopping list — was fully built, never registered.** `bh_lists`/`bh_list_items` (baseline), `services/lists.py` (CRUD), and the `@native_skill("list","lists","shopping-list")` handler all existed but had **no `bh_skills` row**, so nothing could dispatch to it. `0048` registers it (`native://list`) + a `/list` command. → **household-shared** (`0049`: `bh_lists.is_shared` default true; service resolves shared lists household-wide, prefers shared; private `is_shared=false` lists stay creator-scoped; `_user_id` threaded). → **Lists UI**: `pages/ListsPage.tsx` (add / check / remove / clear / list-switcher, optimistic) + REST `routers/lists.py` (`/api/lists`, id-based item ops w/ per-item access control) + a "Lists" nav entry. Chat ("add milk to the list") and the page share one list. 4 DB-backed tests (shared visibility, cross-member check+clear, delete-by-id, private isolation).
- **read-email skill (`0050`)** — email was send-only at the skill layer (a Gmail-IMAP read path lived only in the dashboard widget). New `services/email_reader.py` (sender/subject/date/unread over shared `GMAIL_IMAP_*`) + `@native_skill("read-email","check-email","inbox")` summarizing the inbox. Registered so the LLM router answers "check my email", "any unread", "did I hear from the bank". Read-only.
- **web-search — already done, no work.** `router_engine` L3 **always** attaches Anthropic's native `web_search` tool to the chat turn (`web_search_20250305`, max_uses 5). The assistant already searches for live data. *Optional later:* bump to `web_search_20260209` (dynamic filtering) for Opus 4.6+ — but it must be model-version-gated (errors on older Claude / unsupported on Bedrock), so not done unilaterally.
- **Reminders — already built** (`reminder_parser` + `/remind` + `bh_reminders` + the per-user delivery shipped earlier); no new skill needed.

**State:** tsc clean; 352 frontend tests; backend touched-area sweep green (27); ListsPage is a 4.3KB lazy chunk (main bundle unchanged). Migrations 0047–0050 apply on fresh_db.

- **⚠️ #2 judgment calls left for the owner (not changed):** (a) `/score` vs `/sports` overlap — `/sports` is the richer one (scores+schedules+free-text team); retiring `/score` loses its Detroit per-team shortcut flags, so flagged not deleted. (b) **Structural split:** only ~4 of 21 commands link to a skill; ~18 are special-routed in code while ~19 skills have no command (often the *same* capability — a `weather` skill *and* a special-routed `/weather`). Some special-routed ones are legit app-actions (`/new`,`/help`,`/health`,`/local`); unifying the rest touches `router_engine` and could regress working commands — a separate decision.

- [Next] Owner: decide the two #2 items above; optionally a `/inbox` command + dashboard "unread" deep-link; web_search tool-version bump (model-gated); per-item `added_by` display on the Lists UI (attribution — column exists). Models/affordability deferred per owner.

## [2026-06-28] Command cleanup round 2 + web-search coupling flagged — Claude Code

Follow-up to the assistant pass (owner Q&A on web search / providers / sports). Merged (`cd8f9ee`) + deployed; migrations 0051–0052.

- **`/score` → `/sports` merge (`0051`)** — both already routed to the same `_handle_sports_command`; `/score`'s per-team flags were display-only (never parsed — handler takes free-text teams like `/sports tigers`). Dropped the `/score` row + narrowed the code tuple to `/sports`. Nothing functional lost.
- **`/news` → skill-backed (`0052`)** — proof-of-pattern for collapsing capability commands onto skills: linked `/news` to the `news` skill (`{"category":"$args"}`) and removed the special-route branch + `_handle_news_command`. Clean because the news service already resolves empty category → "top".
- **⚠️ web search is Anthropic-coupled — documented, not changed.** The chat's `web_search` is an Anthropic **server-side** tool (`web_search_20250305`) — only works on Claude models, silently no-ops on Ollama/Bedrock/DeepSeek. Added prominent warnings at the tool def (`router_engine`) and at `_resolve_provider` (`model_provider`). Provider-independent fix = a **client-side** search tool (SearXNG self-hosted / Tavily / Brave) feeding results back as a normal tool_result — a real piece of work, deferred. Owner is **staying on Anthropic** for chat for now.
- **Conversion sweep stopped deliberately.** Checked the rest: `/email` (rich digest/`--clean`/`--unsubscribe` via `inbox_cleaner`), `/recall` (`--list`/`--recent`), `/weather` (flag-word filtering), `/sports` (`--schedule`), `/files` (traversal sanitization) all carry real arg logic a flat `$args` template would regress — and the underlying skills are already LLM-routable for natural language, so nothing is inaccessible. The 18-special-routed/19-skills "split" is mostly by-design, not a bug. **Recommendation (accepted): leave them special-routed**; convert a specific command only by first making its skill robust to raw args. #2 is effectively done (dedup, sports merge, /news).
- **DeepSeek / cheaper providers (owner asked, deferred):** addable (OpenAI-compatible → a `BaseProvider` subclass + prefix rule + `bh_model_rates` row), notably cheaper than Haiku — but cloud privacy tradeoff for finance data, and loses Anthropic web search (needs client-side search first). Recommendation: route high-volume to **Ollama (free+local+private)** before a cheap cloud vendor.

- [Next] Owner: try the new **Lists** nav tab (add/check/remove a grocery list outside chat; shared with Manon — reload the PWA once). Open options if wanted: client-side web search (unlocks provider independence), `/email`→read-email only if the inbox_cleaner power-features move into the skill, per-item `added_by` attribution on the Lists UI.

## [2026-06-28] UMich OLED theme + app-icon library — Claude Code

Owner asks: are themes per-user/device; a UMich dark-OLED theme; "add my icons back as options." Two features built + merged + deployed.

- **Themes are PER-USER (synced across devices)** — stored in `bh_users.settings_json.theme_id`, resolved server-side (user override → platform default → Dark Navy preset). Not per-device. Presets (`is_preset=true, owner_id=NULL`) are options for everyone; each user's pick is independent.
- **Michigan "Maize & Blue" OLED theme (`0053`)** — preset: true-black `#000000` OLED bg, Maize `#ffcb05` primary (on-* foregrounds auto-derive by WCAG contrast → dark button text on maize), readable Michigan-blue accent `#4b8fe2`, dark-blue surfaces. Selectable in Settings → Appearance. Idempotent insert (unique is `(slug, owner_id)`; NULL owners don't collide).
- **App-icon library** — turned the single-active + one-rollback model into a pickable library. Every uploaded icon was *already* archived (`branding/previous` + `branding/history/<ts>`); now surfaced as options. `branding_store.list_library/activate_library_entry/library_preview_bytes` (activate reuses the existing atomic rotation — "previous"→clean swap, history→install+drop dup; `_resolve_library_dir` rejects traversal). Routes: `GET /api/branding/library` (admin), `POST .../{id}/activate` (admin, audited), `GET .../{id}/preview` (**public** — `<img>` can't auth; consistent with the public `/icons/` mount). `IconUploader` gains a "Saved icons" thumbnail grid (Admin → Icon Management) → click to activate. 5 backend tests; IconUploader tests updated for the mount-time library fetch.
...
## [2026-06-29] Lists v2 — deep spec AUTHORED (generic typed-list handler) — Claude Code

Owner broadened the queued "Grocery/Lists v2" into **a best-in-class GENERIC list handler**: many *typed* lists (grocery/chores/gifts/todo/packing), each picked from a dropdown and filterable, with the AI routing free-text items to the right existing list. Added asks during the session: **user-definable custom columns with a column type** (Notion-style: text/number/date/checkbox/single-select/**multi-select**/url), edited in a **per-list settings panel**; **store is a per-item multi-select** tag. Owner weighed Grocy, leaning to keep it native — recommendation given + accepted: **build native, treat Grocy as an idea bank** (Grocy's feature density is the *opposite* of the "too simple but don't make it clunky" goal, and a separate app breaks the AI-native cohesion).

Ran the full `/spec` workflow (manually, since the skill is gated from auto-invoke) at depth **deep**: 3 parallel `spec-researcher` passes → `requirements.md` (30 reqs) → `spec-critic` → **design tournament** (minimal-change / ideal-architecture / risk-first) synthesized into `design.md` → `spec-critic` (caught a real BLOCKER) → `tasks.md` (11 tasks) → traceability validator **30/30 ✓** → final cross-file `spec-critic` (caught the default-list BLOCKER). All findings folded in. Spec lives at **`.kiro/specs/lists-v2/`** (Kiro-compatible).

**Design spine:** a lightweight typed-property engine — `bh_list_field_defs` (core/type/list 3-scope merge, `storage` discriminator) is the single source of truth for storage, validation, the React renderer, AND the AI's view of each list. Typed columns for the hot cross-cutting fields + JSONB `attributes` tail (no EAV). DB-driven `bh_list_types`/`bh_stores`/`bh_store_aisles`/`bh_item_category_aliases` (NO-HARDCODING). AI routing = pgvector cosine of the item vs list embeddings (new `kb_chunks` `source_type='list'`, via a `_reconcile_lists` worker pass) with **DB-tunable thresholds** (placeholders to be calibrated by a fixtured `item→list` test). One additive forward-only migration **`0054`** (schema + stores + seeds + backfill-to-`simple` + the dangerous R2.2 dedupe-then-constrain + `kb_chunks` CHECK widen + config rows). **name→ID API cutover** staged behind name-addressed compat shims (no broken-PWA window); chat path drops auto-create, UI shim keeps it.

**Critic-caught fixes worth remembering:** `kb_chunks` has a CHECK allowing only message/entity (must widen for `'list'`); embedding worker is poll/reconcile, not event-driven (+needs a list reaper or it matches archived/merged lists); the `0.75` threshold was a Haiku probability mis-applied to bge-m3 cosine (placeholder, calibrate); per-list field edits needed an IDOR/core-field-immutability guard; private-list unique index kept case-sensitive to avoid migration abort; `lists.default_list_id` must be elected in-migration + lazily created by the router so the "never drop an item" guarantee holds on a fresh DB.

**State:** spec only — **no code written, nothing deployed.** `grocery-list-v2` memory updated to point at the spec. Implementation is the separate next step.

- [Next] Implement lists-v2 starting at **Task 1 (migration `0054`)**, working tasks top-to-bottom (clean dependency DAG; deploy order migration→backend→frontend), each verified against its Definition of Done. Commit the spec (`.kiro/specs/lists-v2/`) at the handoff.

## [2026-06-29] Lists v2 — IMPLEMENTED (Tasks 1-10 of 11) on branch spec/lists-v2 — Claude Code

Built the whole spec end-to-end in one session (branch **`spec/lists-v2`**, ~11 commits). **Tasks 1-10 done + tested; only Task 11 (threshold calibration) is left and it's Ollama-gated (deploy-box step).** Verified on a throwaway `pgvector/pgvector:pg16` container (host port 55434 — plain postgres:16 lacks the `vector` ext the kb_chunks chain needs). **Full backend suite: 866 passed. Frontend: tsc clean + 352 tests passed.**

- **Task 1 — migration `0054_lists_v2.sql`**: `bh_list_types` + `bh_list_field_defs` (3-scope core/type/list field engine), typed columns on `bh_list_items` (category/due_date/assignee_user_id `ON DELETE SET NULL`/sort_order/attributes jsonb), `bh_stores`/`bh_store_aisles`/`bh_item_category_aliases`, seeds (6 types + core/type field-defs + grocery departments + aliases), backfill to `simple` + fractional sort_order, **kb_chunks CHECK widened to allow `'list'`**, the R2.2 dedupe (merge colliding shared lists into oldest, archive losers, `bh_list_merges` audit) + partial unique indexes (shared case-insensitive, private case-sensitive), routing config + elected default list.
- **Task 2 — `services/list_schema.py`**: effective-schema resolver (merge precedence list>type>core, options resolved from bh_stores/bh_users), `validate_value` per col_type, `partition_item_values` (column vs JSONB by storage), per-list field mutation with the integrity guard (core/type defs never mutated; per-list overrides only).
- **Task 3 — `services/lists.py`**: split `_resolve_list_id`→`resolve_only`(no create)/`create_list`; `add_items_by_id` (validated, no auto-create), fractional `move_item`/`reorder` w/ rebalance, `update_item`, list lifecycle (rename/retype/archive/delete).
- **Task 4 — `routers/lists.py`**: ID-addressed routes + name compat shims (UI shim keeps create-on-add, chat path doesn't), type/config/store endpoints, field endpoints with IDOR/core-immutability guard, confirm-delete. `services/list_config.py` (types/stores/aisles/config).
- **Task 5 — `embedding_worker.py`**: `_reconcile_lists` (embeds name·type·description; item names excluded) + reap of deleted/archived list chunks.
- **Tasks 6+8 — `services/list_router.py`**: pure `_decide` threshold logic, cosine `_score_lists`, `ensure_default_list` (lazy 'Shopping'), `route_item`/`route_and_add` (no junk-list creation, degrades when embeddings down). Removed the hardcoded `"shopping"` default from the skill AND the L3 `manage_list` tool.
- **Task 7 — `services/list_grouping.py`**: grouped/sorted/filtered view (store-aisle order, sort whitelist + schema-validated custom-field sort, per-item multi-select store filter), grocery auto-categorize via alias lookup.
- **Tasks 9-10 — frontend**: `pages/ListsPage.tsx` rewritten schema-driven + ID-addressed (grouped /view, sort + store filter, create-list dialog, optimistic ops, attribute chips); `components/lists/ListSettingsDialog.tsx` (rename/retype/archive/delete + Columns tab + Stores tab). `get_all_lists` now returns id+type+icon.

**Verification ceiling:** the cosine item→list routing and the ≥90% calibration (Task 11) need Ollama and were NOT run here — only the deterministic/degradation/decision paths are tested. Per-item attribute *editing* in the UI is a documented follow-up (the AI + API set attributes today; the grid shows them as chips). The pre-existing `bh_lists.user_id ON DELETE CASCADE` means deleting a member who created a shared list deletes it for everyone — flagged, out of scope (a separate FK decision).

- [Next] (1) **Owner: review/merge `spec/lists-v2`** (not pushed yet — branch is local). (2) On the deploy box (has Ollama): run **Task 11** — seed the labeled item→list fixture, measure the bge-m3 cosine distribution, set `lists.routing` thresholds in `bh_platform_settings`. (3) Deploy in staged order migration→backend→frontend (name shims keep cached PWAs working). (4) Optional follow-ups: per-item attribute editor UI; route hook_engine push through notify_users; the user_id-cascade FK decision.

## [2026-06-29] Lists v2 — UI editing affordances added + DEPLOYED to prod — Claude Code

Owner asked to add 6 UI gaps then push+deploy to test prod. Added (commit `fe15465` on `spec/lists-v2`, pushed; PR #49):
1. **ItemEditorDialog** — edit any item's fields by schema (text/number/date/select/multi-select/url + assignee + due date + store tags). 2. **Select-column options** — define on create + edit inline-option selects in Settings→Columns. 3. **Drag-to-reorder** on flat (ungrouped) manual-sort lists → reorder endpoint. 4. inline edit (pencil per row). 5. **Archived-lists dialog** (`GET /api/lists?archived=true`) + Restore. 6. **Store aisle-order editor** in Settings→Stores → PUT aisles. Backend: `get_all_lists` archived flag + always returns `lists` key; `?archived` param; **due_date ISO strings coerced to datetime** (`_coerce_due` — asyncpg binds datetimes not strs) on add+update. 58 list backend tests + 352 frontend tests pass; tsc clean.

**DEPLOYED** `bowershub-ai` to prod (server, branch `spec/lists-v2`). Pre-deploy safety: targeted `pg_dump` of `finance` → `/home/michael/backups/pre-lists-v2/pre-lists-v2-20260629-223203.dump` (5.5M). Migration **0054 applied clean on the live DB**: 6 seed types, the 1 existing list backfilled (0 untyped), **0 dedupe merges** (no colliding shared names — prod data was clean), item preserved, kb_chunks CHECK widened. Health `ok`, new routes 401 (registered+gated). **Prod is ready to test.**

- **Still pending:** Task 11 calibration needs Ollama on the box (placeholder `lists.routing` thresholds live in `bh_platform_settings` now) — the cosine routing won't be well-tuned until then; **list embeddings also need the embedding worker to tick** (deployed, runs on schedule). Branch not merged to `main` yet (PR #49 open). The `bh_lists.user_id` CASCADE edge still open.

## [2026-06-30] Lists v2 — table/inline-edit UI commits journaled + Task 11 calibration DONE — Claude Code

Closing out lists-v2. Two parts: backfilling the journal for 4 UI commits that shipped after the last entry, and completing Task 11 (routing threshold calibration) — the last open task in the spec.

**UI commits (already on `spec/lists-v2` + deployed to prod, were unlogged):**
- `16f96cf` **Table view** — List/Table toggle; table mode renders the list's schema columns as real columns (item, category/department, assignee, due date, store, custom fields — select value→label resolved), grouped sections become subheader rows. A "Columns" editor toggles visibility. View mode + hidden columns persist per-list in localStorage.
- `128d64b` **Prominent toggle** — labeled List/Table toggle (hidden on mobile), filled-primary active state so it reads as a real control. + SW_VERSION bump.
- `bf13c41` **PWA SW bump** — the table-view bundle had shipped without a SW_VERSION bump so installed PWAs kept the old bundle; bump forces re-activate (cache wipe + clients.claim).
- `f4e3b40` **Inline cell editing** — click any table cell to edit in place (text/number/date/url inputs; single-select + checkbox inline; category resolves type departments; multi-select opens the full editor). Each edit PATCHes one field + re-fetches so grouping/sorting update. Replaces the per-row pencil in table view.

**Task 11 — routing calibration DONE (`25f58da`).** Ran real bge-m3 calibration on the box (Ollama local). **Key finding (a real design issue, not just a number):** the as-built routing document — `name · type_label · LIST.description` — was too thin. Real lists rarely have a per-list description, so the embedded text was often just `"Groceries · Grocery · "`, giving **~71% accuracy with genuine mis-routes** (e.g. swimsuit→Hardware) and NO threshold that separated correct matches from noise (correct sims 0.38–0.59 overlapped noise at 0.39).
- **Fix (migration `0055` + `embedding_worker._LIST_CONTENT_SQL`):** fold the **TYPE** description into the embedded document via `concat_ws`, and enrich the seeded `bh_list_types.description` with representative item terms (DB-driven, NO-HARDCODING). Stable **type-level** terms — NOT churning per-list item names — so no re-embed storm; the original "item names excluded" decision stands in spirit. Lifts routing to **90.3%** (0 noise-leak, 1 mis-route) with clean separation → thresholds calibrate at **match=0.40 / ambiguity_margin=0.04** (was the 0.55/0.07 placeholder). The 0.55 placeholder was ~3× too high for bge-m3 cosine.
- **`list_config.routing_config` fallback default** also bumped to 0.40/0.04 (for any DB lacking the row).
- **Tradeoff noted:** bge-m3 margins are tight, so ~1/3 of items hit the disambiguation prompt ("which list?") rather than auto-routing — deliberate (better to ask than mis-route). am<0.04 trades accuracy for mis-routes; am>0.04 just adds nagging.
- **Verified 3 ways:** (1) calibration script vs real Ollama; (2) deterministic CI-safe fixture test `test_list_routing_calibration.py` (replays captured real bge-m3 cosine through the actual `list_router._decide`; also asserts the OLD placeholder *fails* the goal, so a silent revert breaks loudly); (3) **live end-to-end through real pgvector halfvec** (seed lists → real embed of the `concat_ws` doc → halfvec store → `route_item` → `_decide`): **6/6** correct, incl. swimsuit→Trip Packing that failed on the old doc. halfvec float16 precision is empirically a non-issue at these margins.
- **Migration 0055 guards:** every `UPDATE` is conditioned on the prior value (type descriptions on the exact old seed text; thresholds on the exact placeholder jsonb), so re-running is idempotent and an owner's UI edits are never clobbered. Changing the type descriptions bumps each list's content hash → the embedding worker re-embeds all lists on its next tick (intended).
...
## [2026-06-30] Skill Chaining — L2 multi-source gather before L3 (last design-review proactivity item) — Claude Code

Built the remaining design-review item (`feat/skill-chaining`). **Research first:** the L2 `tool_router` already chains up to 3 **API-registry** calls (`action:"api_call"`), but does NOT expose the **native skills** (email/calendar/weather/sports/lists) — so multi-source asks like "search my email and calendar for flights" still escalate to expensive L3 Opus. The gap = native read-only skill chaining.

**Implementation** — a new **Layer 2.7** in `router_engine` (where `skill_executor` + user/workspace context already live), inserted after the API tool-router and before L3:
- `_try_skill_chain`: Haiku plans which **READ-ONLY** skills to call (the 11 `is_read_only=true` skills — weather/read-email/sports-score/balances/transactions/recall/etc.; DB-driven), executes them via `skill_executor.execute`, then Haiku synthesizes one answer. Returns `RoutingResult(layer="L2", skill_name="skill_chain")`.
- **Read-only only** → no write side effects, safe to run speculatively. **Multi-source only** (≥2 calls) — single-source stays with the existing L2 classifier. **Fail-safe**: any error / non-JSON plan / <2 skills → returns None → falls through to L3 exactly as before (whole layer wrapped in try/except in `route()`). `workspace_skills` now initialized to None before the L2 try so it's always bound for the new layer.
- **Cost:** one extra Haiku plan-call only for messages that were already L3-bound (handled ones exit at L1/L2 earlier) — cheap vs the Opus call it can now avoid.
- **Tests:** 5 (too-few-readonly→None, single-source→None, multi-source gather+synthesize with invalid-skill filtering, one-skill-failure-still-synthesizes, bad-JSON→None). Router suites green (28 passed, no regressions). No migration.
- [Next] Push + PR + deploy. **Watch:** whether Haiku reliably recognizes genuine multi-source asks and picks the right skills (the real-world quality question, like capture recall) — tune the plan prompt from observed behavior; the `skill_chain` skill_name in routing telemetry shows when it fires. This was the LAST design-review proactivity item; both (Context Harvester + Skill Chaining) now done.

## [2026-06-30] Frontend hardening (design-review Track 2) — typed + FormData-aware api client, kill raw fetches — Claude Code

Track 2 of the design-review follow-through (branch `feat/frontend-hardening`, off `main` — independent of Track 1). The review flagged "463 anys" + "hand-rolled fetch chains." On inspection the boundary was already half-built (Zod `src/schemas/*` + `lib/validate.parseLoose` in ~8 stores), so the real gap was: `api.ts request` returned `any`, and **12 raw `fetch()` calls bypassed the client entirely** — each manually grabbing the token and **none doing the 401-refresh** the client does, so an expired token made uploads/gallery ops hard-fail. The comment in IconUploader said it all: "Uses fetch directly because the shared `api` client only handles JSON bodies."

**Built:**
- **`api.ts` is the typed choke point:** `request<T>`/`get/post/put/patch/delete<T>` are now generic (default `T=any` so all 178 existing call sites keep compiling — typing is opt-in, `api.get<Foo>()`; validate with `parseLoose` where shape matters). Return is `{ data: T; status }`.
- **FormData support:** if the body is `FormData`, the client sends it as-is with **no** `Content-Type` (browser sets the multipart boundary) instead of forcing JSON. This is what let the upload fetches move onto the client.
- **Routed 12 fetches through the client** (auth + 401-refresh + consistent error shape, ~−74 LOC): IconUploader (icon upload + revert/rollback + library), QuickCaptureOverlay + QuickCapturePage (image upload), ImageGallery (fetch/upload/reorder/set-primary/unlink), UndoRedoProvider (clear-session via extra-headers), db-browser `importCsv` (it already hand-rolled the client's exact `{response:{status,data}}` throw shape).
- **Left raw, intentionally (documented):** `stores/auth.ts` refresh/logout (would recurse through the client), `useDashboardStream` (SSE stream), `useDashboardWidget` (AbortController timeout — client has no signal arg), db-browser `export-csv` (blob download), `lib/reportError` (telemetry keepalive). These are genuine special cases, not the JSON-API boundary.

**Tests/verify:** new `services/__tests__/api.test.ts` (3 — JSON sets content-type+token+stringified body; FormData passed through with NO content-type; `{data,status}` envelope). Existing `IconUploader.test.tsx` (mocks global fetch) still green through the new client path. Full FE suite **355 passed** (was 352 + 3), tsc clean, build clean. Zero raw `fetch(` left in `src/components` + `src/pages`.

**Honest scope:** this delivers the typed+FormData-capable boundary and centralizes auth/401 — NOT the full per-endpoint `any` reduction (that's incremental: adopt `api.get<T>` + a schema per endpoint as files are touched) and NOT TanStack Query (the plan marked it optional — only ~7 useEffect+fetch files, not worth it yet).
- [Next] Push + PR `feat/frontend-hardening`. Then **Track 3** (deprecate/sunset n8n: port live `n8n-workflows/` to Python `apscheduler`, swap hardcoded `100.106.180.101` in `n8n-workflows/`+`dashboard/` for env/Docker-DNS, decommission the n8n container, delete stray `frontend/vite.expose.config.ts`). Track 1 (`feat/captured-fact-privacy`) and Track 2 are both unpushed/unmerged — owner to decide merge/deploy order.

## [2026-06-30] CI green — gitleaks false-positive baseline + backend flake hardening — Claude Code

`main`'s CI had been red on every commit. Root-caused two independent failures (branch `chore/ci-green`):

**1. Secret scan (gitleaks) — the consistent red.** `gitleaks detect` scans full git history; it flagged 2 findings in `backend/tests/test_provisioning.py` at commit `cbac0965` — both throwaway TEST admin-password literals in an `ADMIN_PASSWORD=...` assignment (value not reproduced here, to avoid re-tripping the scanner), NOT real secrets (the current code already extracts them to `_FIRST_ADMIN_PW` to dodge the scanner, but the old commit still trips it). Fix: added `.gitleaksignore` with the 2 exact fingerprints (precise — still catches any NEW real secret). NOTE: the first attempt quoted the literal value in `.gitleaksignore` + this log, which created 2 NEW findings in this branch's own commit — reworded to never reproduce the literal. Verified with gitleaks v8.21.2 locally → `no leaks found`. No rotation needed (nothing real leaked).

**2. Backend suite — intermittent flake.** Failed on some main runs, passed on others (incl. the latest post-merge run + 883/883 locally). The failing runs' logs were purged before I could see the test, so I hunted by pattern and found a real race in `test_model_catalog_refresh.py::test_single_flight_and_audit_log`: it coordinated two concurrent `refresh()` tasks with fixed `sleep(0)`/`sleep(0.02)`, but `refresh()` acquires the DB pool before `discover()`, so under CI load t1 may not have reached the gate when the assertion ran → `gated.calls` 0 not 1. Fix: made it deterministic — `FakeSource` now sets an `entered` event when `discover()` begins; the test `await`s that (t1 definitely at the gate) then drains the loop with `sleep(0)` (no wall-clock wait) before asserting t2 is lock-blocked. Stress-ran 20×0 fails. **Can't prove this was THE flake** (logs gone), but it's a genuine race fixed regardless.
...
**Task 1 (inventory, prod-grounded → `inventory.md`):** dispatch is name-first (verified `skill_executor.py:146-148`) → of 11 `/webhook/` `bh_skills` rows, **8 already run native** (inert URL); only `smart-capture-extract/commit` + `process-asset` truly hit n8n. **`api_usage_log` decommission gate ALREADY CLEARED** — native `cost_tracker.py` is the live writer (n8n stopped 2026-06-07); no new logger needed (design flagged it as a blocker; prod data resolved it). Caveat logged: native logger only records finance/ask-db spend, not general chat — pre-existing, orthogonal to n8n.

**Tasks 2–9 (code, landed DARK — `smart_capture.engine` defaults `'n8n'`, so everything still proxies; zero behavior change until cutover):**
- New `services/smart_capture/`: `config` (DB engine switch + HMAC secret + gates), `intents` (typed intent + single asset-bound `canonical()/hash()`), `tokens` (**real HMAC-SHA256** — tamper/expiry/uid+wid/membership; replaces n8n's decorative djb2), `prompt` (classify prompt + DOMAINS verbatim), `extract` (one governed model call + fence-strip/other-fallback + input bounds + token mint), `commit`+`committers` (verify → `bh_smart_capture_commits` idempotency → per-domain committer; **all SQL parameterized**; DB domains atomic incl. asset-link → no orphan; shopping_list→`list_router`, knowledge_fact/project/other→`knowledge.remember`), `process_asset` (native vision: probe→sha256 dedup→insert→vision→move→update, parameterized — **fixes n8n's interpolated SQL**), `engine` (native / n8n-proxy / shadow; shadow returns n8n body but re-mints a native token over n8n's intents).
- Wiring: `services/skills/smart_capture.py` `@native_skill` wrappers (name-first dispatch = the cutover); `skill_executor` injects `_workspace_id/_config/_model_provider` (cached; non-breaking); `db_browser` `/inbox/ai-extract`+`url-extract` routed through dispatch (were bypassing it), url-extract keeps exact pass-URL-as-text (no scrape), workspace resolved at runtime (no hardcoded id); removed `_get_smart_capture_url` + `httpx`/`get_http_session` imports. Migration `0058` (engine setting, `gen_random_bytes` secret, gates, dedup table) — idempotent, green under scoped migrator role + fresh build-from-empty.
- **Tests: 60 new deterministic tests**; **full backend suite 940 passed, 0 failures** (throwaway pgvector pg16 on :5433).
- **email-receipts-importer: DORMANT → deferred** (0 email/receipt txns in 60d; depends on unported `receipt-to-transaction` + live Gmail/IMAP-trigger; can't verify without live email; owner can live without it). Not half-built — port-or-drop at S5. See `inventory.md`.
