...
- [Tracked follow-up — PRE-EXISTING failures, NOT caused by C2]: 5 remain red and predate this work — `test_db_browser_images::test_get_row_images_with_results`, `test_finance_endpoints::{summary_success,balances_success,balances_null_balance}` (dashboard finance query `KeyError 'mtd_income'/'last_balance'` against empty finance data), `test_layout_persistence::test_property_layout_persistence_round_trip`. Must be resolved (fix or xfail-with-reason) before the full-suite CI gate can be green.
- [Next for C2]: (a) flip the CI backend job (`TODO(C2)` in ci.yml) to a Postgres service + full `pytest` (the from-empty gate) once the 5 pre-existing are handled; (b) **off-site backup verification** (the second half of C2 — confirm nightly dump + remote/off-site actually runs and restore-tests); (c) decide merge + deploy timing (owner's call — safe per validation #4).
