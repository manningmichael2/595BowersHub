...
- [BLOCKER][need from owner]: a schema dump of the live DB — `docker exec <pg-container> pg_dump -U <DB_USER> --schema-only --no-owner --no-privileges <DB_NAME> > live_schema.sql` — dropped in the repo (treated as throwaway, **not committed**) or pasted. `DB_HOST=postgres` is the on-server docker-network name, so this sandbox can't reach the live DB; only the owner can produce it. This is the source for the layer-0 base tables AND the validation oracle. **C2 is paused here until that dump exists.** Off-site backup verification (the other half of C2) is still pending too.
