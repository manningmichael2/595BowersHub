...
1. **Token format: keep `--color-X` as full color; add a *derived* parallel triple var `--color-X-rgb` for alpha. DB keeps hex.** *(minimal-change won; **revised during T1 implementation** — see note.)*
   The ideal-arch lens says "store channel triples in `bh_themes.tokens_json`." But that breaks `ThemeBuilder`'s hex pickers and the on-primary luminance math, and makes the DB unreadable. The first cut of this design said: keep DB hex, convert to a triple *in place* in the `--color-X` var, and wrap every direct `var(--color-X)` consumer in `rgb(...)`. **Implementation revealed that blast radius is 837 direct `var(--color-X)` usages across 59 files** (`grep -rho 'var(--color-[a-z-]*)' src`) — not "finite and greppable," but essentially every inline style in the app. Wrapping all 837 is exactly the kind of churn that got the prior redesign reverted.
   **Revised approach:** `--color-X` **stays a full color value** (hex), so all 837 existing consumers keep working with **zero edits**. `App.tsx` *additionally* injects a derived `--color-X-rgb` space-separated `"R G B"` triple from the same hex; `tailwind.config.ts` maps each color to `rgb(var(--color-X-rgb) / <alpha-value>)`. This makes `bg-primary/20` render correct alpha (R1.4) while full-opacity colors are provably unchanged.
   **Why this still satisfies R1.1 (no second authority):** `--color-X-rgb` is *derived at injection from the one `bh_themes` row* — a second *representation*, not a second *authority* — precisely like `--color-on-primary` and `--color-surface-light/dark`, which `App.tsx` already derives today. The DB remains the sole source of truth. No DB format migration needed (only the R1.2 warning/error key-add). During P4, surfaces that migrate from inline `var(--color-X)` styles to Tailwind classes pick up the alpha-composable path naturally; a later cleanup can retire `--color-X` if inline usage reaches zero, but that is explicitly **not** required by this spec.
...
| Risk | Mitigation |
|------|------------|
| Channel-triple switch could break direct `var(--color-x)` consumers (found: **837 across 59 files**) | **Resolved via Strategy B:** `--color-x` stays a full color; a *derived* `--color-x-rgb` triple is added for Tailwind only. Zero direct-consumer edits. Rendered-alpha check (not class compilation) is the gate; full-opacity color provably unchanged (`rgb(99 102 241 / 1)` === `#6366f1`). |
