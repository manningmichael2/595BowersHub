...
- [x] Confirm primitives/shell size **type** with scaling `text-*` (resolving to `--bh-text-base`) and **layout** with fixed-rem utilities; no hardcoded px font-size (R1.6). **Verified 2026-07-01:** `components/ui` + `components/shell` carry zero `text-[Npx]`/inline `fontSize` and use the `text-*` scale (39× `text-sm` etc.); `index.css` derives every `text-*` from `calc(var(--bh-text-base) * k)`. Guarded by `alphaScaling.test.ts`.
- [x] **Tests:** `hexToTriple`/`setColorVar` unit tests (`themeTokens.test.ts`) + `alphaScaling.test.ts` (10) — asserts the tint-bearing tokens (`primary`/`accent`/`danger`/`success`/`warning`/`error`) map through their `-rgb` triple so `bg-x/<n>` renders a tint, the two known-broken call-sites (`MessageList`/`SearchOverlay`) now carry `bg-primary/<n>`, `--bh-text-base` single-sources every `text-*`, and no hardcoded font-size in primitives/shell. `tsc --noEmit` clean; 365 tests green. **Deferred to Task 14 (Playwright, owner-approved):** true *computed-style* colour resolution and *overflow-at-four-sizes* — jsdom compiles no Tailwind CSS and does no layout, so those are visual-harness checks, not CI ones.
