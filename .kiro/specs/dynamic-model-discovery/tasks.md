...
## Task 8: Consolidate the cost path + cost-parity gate (phase P3) — ✅ DONE
- **Effort:** M
- **Dependencies:** Task 5
- **Requirements:** R3.3, R3.4
- **Outcome:** `cost_for(model_id, in, out)` in `model_catalog.py` is the single cost home — exact-match catalog price (incl. inactive rows; resolver cache, no DB hit), same-provider normalize fallback, then a non-zero name-heuristic floor + WARN on miss/NULL (never 0). Resolver-optional (safe in tests/early startup). `RouterEngine._calculate_cost` is now a 1-line delegator to it (heuristic deleted; all 3 call sites — `:239/:1203/:1436` — route through the one home); dead `CostTracker.calculate_cost` removed. Tests `tests/test_model_cost.py` (2): miss-path **byte-identical** to the legacy heuristic across a model×token grid + never-0; DB price authoritative; Bedrock exact-match (B1); inactive row still prices. **25/25** model tests green.
- **Cost-parity diff report (human-review gate, generated against live DB):** Claude cloud models are **cost-neutral** (DB rate == heuristic for haiku/sonnet/bedrock). Intentional **corrections**, not regressions: (1) local/Ollama models go from phantom $3/$15 (old heuristic wrongly treated unknown names as Sonnet) → **$0** (free self-hosted); (2) Opus (via the 0005-inserted canonical row, used post-Task-9) goes from stale $15/$75 → current **$5/$25**. No model is mis-priced low or silently 0. Safe to cut over.
...
## Task 9: Delitteralize tier config + inline literals + default model (phase P4) — ✅ DONE
- **Effort:** M
...
- **Outcome:** Added module-level `resolve_role(role)` / `default_chat_model()` to `model_catalog.py` (resolver-cache when warmed, documented canonical cold-start fallback otherwise — never raises; `model_catalog.py` is the single home for residual literals). Repointed every live site: 8 `self.config.HAIKU_MODEL` + the `:670` local literal in `router_engine.py`; the inline literals in `finance.py` (ask_db), `context_capture.py`, `hook_engine.py`, `tool_router.py`, `scheduled_prompts.py`, `categorizer.py`, `knowledge.py`, `inbox_cleaner.py`, `local_intelligence.py`; **removed the `os.environ.get` model overrides** (tool_router/local_intelligence — DB is authoritative); deleted the now-unused `config.py` HAIKU/SONNET/LOCAL_MODEL constants; `get_default_chat_model()` delegates to `default_chat_model()` (R4.4). `model_catalog.py` has **zero backend imports** so no cycle. All 12 touched files compile; **489 tests collect with no import errors**; model-discovery suite + scheduled_prompts + ask_db sandbox green (35 + 2). Remaining residual literals (`model_provider._fallback_models` :170/:173, dead post-Task-7) are removed in Task 11 with the grep gate.
